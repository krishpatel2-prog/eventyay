...
class Voucher(LoggedModel):
    ...
    * If you select an product that has variations but do not select a variation, you cannot set block_quota
    ...
    tag = models.CharField(
        ...
        db_index=True,
        help_text=_(
            'You can use this field to group multiple vouchers together. If you enter the same value for '
    ...
    def annotate_budget_used_orders(cls, qs):
        opq = (
            ...
            .order_by()
            .values('voucher_id')
            .annotate(s=Sum(F('price_before_voucher') - F('price')))
            .values('s')
        )
        return qs.annotate(
            budget_used_orders=Coalesce(
                Subquery(
                    opq,
