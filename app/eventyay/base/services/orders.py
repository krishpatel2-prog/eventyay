...
error_messages = {
    ...
    'price_changed': _(
        'The price of some of the products in your cart has changed in the meantime. Please see below for details.'
    ),
    'internal': _('An internal error occurred, please try again.'),
    'empty': _('Your cart is empty.'),
    'max_products_per_product': _(
        'You cannot select more than %(max)s products of the product %(product)s. We removed the '
        'surplus products from your cart.'
    ),
    ...
    'voucher_invalid': _('The voucher code used for one of the products in your cart is not known in our database.'),
    'voucher_redeemed': _(
        'The voucher code used for one of the products in your cart has already been used the maximum '
        'number of times allowed. We removed this product from your cart.'
    ),
    'voucher_budget_used': _(
        'The voucher code used for one of the products in your cart has already been too often. We '
        'adjusted the price of the product in your cart.'
    ),
    'voucher_expired': _(
        'The voucher code used for one of the products in your cart is expired. We removed this product from your cart.'
    ),
    'voucher_invalid_product': _(
        'The voucher code used for one of the products in your cart is not valid for this product. We '
        'removed this product from your cart.'
    ),
    'voucher_required': _(
        'You need a valid voucher code to order one of the products in your cart. We removed this product from your cart.'
    ),
    'some_subevent_not_started': _(
        'The presale period for one of the events in your cart has not yet started. The '
    ...
    'seat_unavailable': _(
        'One of the seats in your order has been taken in the meantime, we removed the position from your cart.'
    ),
...
def reactivate_order(order: Order, force: bool = False, user: User = None, auth=None):
    ...
    if (
        num_invoices > 0
        and order.invoices.filter(is_cancellation=True).count() >= num_invoices
...
def extend_order(order: Order, new_date: datetime, force: bool = False, user: User = None, auth=None):
    ...
    def change(was_expired=True):
        ...
        order.log_action(
            'eventyay.event.order.expirychanged',
            user=user,
            auth=auth,
            data={'expires': order.expires, 'state_change': was_expired},
        )
...
def mark_order_expired(order, user=None, auth=None):
    ...
    with transaction.atomic():
        ...
        order.log_action('eventyay.event.order.expired', user=user, auth=auth)
...
def approve_order_without_side_effects(order, user=None, auth=None, force=False):
    with transaction.atomic():
        ...
        order.log_action('eventyay.event.order.approved', user=user, auth=auth)
...
def send_order_approved_notifications(order, invoice=None, user=None, send_mail: bool = True):
    order_approved.send(order.event, order=order)
        ...
        with language(order.locale, order.event.settings.region):
            ...
            try:
                order.send_mail(
                    ...
                    email_template,
                    email_context,
                    'eventyay.event.order.email.order_approved',
                    user,
...
def approve_order(order, user=None, send_mail: bool = True, auth=None, force=False):
    ...
    :param user: The user that performed the change
    """
    invoice = approve_order_without_side_effects(order, user=user, auth=auth, force=force)
    send_order_approved_notifications(order, invoice=invoice, user=user, send_mail=send_mail)

    return order.pk


def deny_order_without_side_effects(order, comment='', user=None, auth=None):
    with transaction.atomic():
        if not order.require_approval or not order.status == Order.STATUS_PENDING:
            raise OrderError(_('This order is not pending approval.'))
        ...
        order.log_action('eventyay.event.order.denied', user=user, auth=auth, data={'comment': comment})
        i = order.invoices.filter(is_cancellation=False).last()
        if i:
            generate_cancellation(i)
...
def send_order_denied_notifications(order, comment='', user=None, send_mail: bool = True):
        ...
        with language(order.locale, order.event.settings.region):
            ...
            try:
                order.send_mail(
                    ...
                    email_template,
                    email_context,
                    'eventyay.event.order.email.order_denied',
                    user,
                )
            except SendMailException:
...
def deny_order(order, comment='', user=None, send_mail: bool = True, auth=None):
    ...
    :param user: The user that performed the change
    """
    deny_order_without_side_effects(order, comment=comment, user=user, auth=auth)
    send_order_denied_notifications(order, comment=comment, user=user, send_mail=send_mail)
            ...
            with language(order.locale, order.event.settings.region):
                ...
                try:
                    order.send_mail(
                        ...
                        email_template,
                        email_context,
                        'eventyay.event.order.email.order_canceled',
                        user,
...
def _check_positions(
    ...
    for i, cp in enumerate(sorted(positions, key=lambda s: -int(s.is_bundled))):
        ...
        if not cp.product.is_available() or (cp.variation and not cp.variation.active):
            err = err or error_messages['unavailable']
            delete(cp)
            continue
        quotas = list(cp.quotas)

        products_seen[cp.product] += 1
        if cp.product.max_per_order and products_seen[cp.product] > cp.product.max_per_order:
            err = error_messages['max_products_per_product']
            errargs = {'max': cp.product.max_per_order, 'product': cp.product.name}
            delete(cp)
            break
        ...
        if (
            (cp.requires_seat and not cp.seat)
            or (cp.seat and not cp.requires_seat)
            or (cp.seat and cp.seat.product != cp.product)
            or cp.seat in seats_seen
        ):
        ...
        if cp.product.require_voucher and cp.voucher is None and not cp.is_bundled:
            delete(cp)
            err = err or error_messages['voucher_required']
            break

        if (
            cp.product.hide_without_voucher
            and (
                cp.voucher is None
                or not cp.voucher.show_hidden_products
                or not cp.voucher.applies_to(cp.product, cp.variation)
            )
        ...
        if cp.seat:
            ...
            if not cp.seat.is_available(
                ignore_cart=cp,
                ignore_voucher_id=cp.voucher_id,
        ...
        if (
            cp.subevent
            and cp.variation
            and cp.variation.pk in cp.subevent.var_overrides
            and cp.subevent.var_overrides[cp.variation.pk].disabled
        ):
            err = err or error_messages['unavailable']
            delete(cp)
            continue
        ...
        if price.gross != cp.price and not (cp.product.free_price and cp.price > price.gross):
...
def _get_fees(
    ...
    for recv, resp in order_fee_calculation.send(
        sender=event,
        invoice_address=address,
        total=total,
        meta_info=meta_info,
...
def _create_order(
    ...
    with transaction.atomic():
        ...
        if checked_gift_cards and any(c.product.issue_giftcard for c in positions):
            raise OrderError(_('You cannot pay with gift cards when buying a gift card.'))

        fees, pf, gift_card_values = _get_fees(
            positions, payment_provider, address, meta_info, event, checked_gift_cards
        )
        ...
        for gc, val in gift_card_values.items():
            ...
            trans = gc.transactions.create(value=-1 * val, order=order, payment=p)
            p.info_data = {
                'gift_card': gc.pk,
        ...
        if shown_total is not None:
            if Decimal(shown_total) != pending_sum:
                raise OrderError(
                    _(
                        'While trying to place your order, we noticed that the order total has changed. Either one of '
        ...
        order.log_action('eventyay.event.order.placed')
        if order.require_approval:
            order.log_action('eventyay.event.order.placed.require_approval')
        if meta_info:
...
def _order_placed_email(
    event: Event,
    order: Order,
    pprov: BasePaymentProvider,
    email_template,
    log_entry: str,
    invoice,
    ...
    if email:
        ...
        if product_ids:
            ...
            if flagged_product_ids:
                existing = OrderPosition.objects.filter(
                    order__event=event,
                ...
                if existing:
                    raise OrderError(
                        _('Only one ticket per user is allowed for this product. You already have a ticket.')
                    )
...
def expire_orders(sender, **kwargs):
    ...
    for o in (
        Order.objects.filter(expires__lt=now(), status=Order.STATUS_PENDING, require_approval=False)
...
def send_expiry_warnings(sender, **kwargs):
    ...
    for o in (
        ...
        .only('pk', 'event_id', 'expires')
        .order_by('event_id')
    ):
        if event_id != o.event_id:
            settings = o.event.settings
            days = cache.get_or_set(
                '{}:{}:setting_mail_days_order_expire_warning'.format('event', o.event_id),
...
def send_download_reminders(sender, **kwargs):
    ...
    qs = (
        Order.objects.annotate(first_date=Coalesce(Min('all_positions__subevent__date_from'), F('event__date_from')))
        .filter(
            download_reminder_sent=False,
            datetime__lte=now() - timedelta(hours=2),
            first_date__gte=today,
        )
        .only('pk', 'event_id', 'sales_channel')
        .order_by('event_id')
    )
    event_id = None
    days = None
    ...
    for o in qs:
        ...
        with transaction.atomic():
            ...
            if not o.ticket_download_available:
                continue
            positions = o.positions.select_related('product')

            if o.status != Order.STATUS_PAID:
                if (
                    o.status != Order.STATUS_PENDING
                    or o.require_approval
                    or not o.event.settings.ticket_download_pending
                ):
                    continue
            send = False
            for p in positions:
            ...
            with language(o.locale, o.event.settings.region):
                ...
                try:
                    o.send_mail(
                        ...
                        email_template,
                        email_context,
                        'eventyay.event.order.email.download_reminder_sent',
                        attach_tickets=True,
                    )
                except SendMailException:
                    logger.exception('Reminder email could not be sent')
    ...
    with language(order.locale, order.event.settings.region):
        ...
        try:
            order.send_mail(
                ...
                email_template,
                email_context,
                'eventyay.event.order.email.order_changed',
                user,
                auth=auth,
                invoices=invoices,
                attach_tickets=True,
...
class OrderChangeManager:
    ...
    ProductOperation = namedtuple('ProductOperation', ('position', 'product', 'variation'))
    ...
    def change_product(self, position: OrderPosition, product: Product, variation: ProductVariation | None):
        ...
        new_quotas = (
            variation.quotas.filter(subevent=position.subevent)
            if variation
            else product.quotas.filter(subevent=position.subevent)
        )
        if not new_quotas:
        ...
        self._operations.append(self.ProductOperation(position, product, variation))
            ...
            if self.event.has_subevents:
                ...
                for p in self._operations:
                    if isinstance(p, self.SubeventOperation) and p.position == position:
                        subev = p.subevent
            try:
                seat = Seat.objects.get(event=self.event, subevent=subev, seat_guid=seat)
    ...
    def change_subevent(self, position: OrderPosition, subevent: SubEvent):
        ...
        new_quotas = (
            position.variation.quotas.filter(subevent=subevent)
            if position.variation
            else position.product.quotas.filter(subevent=subevent)
        )
        if not new_quotas:
            raise OrderError(self.error_messages['quota_missing'])
    ...
    def change_product_and_subevent(
        self,
    ...
    def change_price(self, position: OrderPosition, price: Decimal):
        ...
        if (
            self.order.event.settings.invoice_include_free
            or price.gross != Decimal('0.00')
            or position.price != Decimal('0.00')
        ):
            self._invoice_dirty = True
    ...
    def _current_tax_rules(self):
        ...
        for p in self._operations:
            ...
            elif isinstance(p, self.ProductOperation):
                tax_rules[p.position.pk] = p.product.tax_rule
        return tax_rules

    def recalculate_taxes(self, keep='net'):
        positions = self.order.positions.select_related('product', 'product__tax_rule')
        ...
        for pos in positions:
            ...
            if new_rate != pos.tax_rate:
                if keep == 'net':
                    new_tax = tax_rule.tax(
                        pos.price - pos.tax_value,
                        base_price_is='net',
    ...
    def add_position(
        ...
        if isinstance(seat, str):
            if not seat:
                ...
                try:
                    seat = Seat.objects.get(event=self.event, subevent=subevent, seat_guid=seat)
                except Seat.DoesNotExist:
                    raise OrderError(error_messages['seat_invalid'])
        ...
        try:
            if price is None:
                ...
                price = product.tax(price, base_price_is='gross', invoice_address=self._invoice_address)
        except TaxRule.SaleNotAllowed:
            raise OrderError(self.error_messages['tax_rule_country_blocked'])

        if price is None:
            raise OrderError(self.error_messages['product_invalid'])
        if not addon_to and product.category and product.category.is_addon:
            raise OrderError(self.error_messages['addon_to_required'])
        if addon_to:
            if not product.category or product.category_id not in addon_to.product.addons.values_list(
        ...
        seated = product.seat_category_mappings.filter(subevent=subevent).exists()
        ...
        if seat:
            self._seatdiff.update([seat])
        self._operations.append(self.AddOperation(product, variation, price, addon_to, subevent, seat))
    ...
    def _check_paid_to_free(self):
        if (
            self.order.total == 0
            and (self._totaldiff < 0 or (self.split_order and self.split_order.total > 0))
            and not self.order.require_approval
        ):
    ...
    def _perform_operations(self):
        ...
        for op in self._operations:
            if isinstance(op, self.ProductOperation):
                self.order.log_action(
                    'eventyay.event.order.changed.product',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'position': op.position.pk,
                        'positionid': op.position.positionid,
                        'old_product': op.position.product.pk,
                        'old_variation': op.position.variation.pk if op.position.variation else None,
                        'new_product': op.product.pk,
                ...
                op.position.product = op.product
                ...
                self.order.log_action(
                    'eventyay.event.order.changed.seat',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'position': op.position.pk,
                ...
                self.order.log_action(
                    'eventyay.event.order.changed.subevent',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'position': op.position.pk,
                ...
                self.order.log_action(
                    'eventyay.event.order.changed.addfee',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'fee': op.fee.pk,
                ...
                self.order.log_action(
                    'eventyay.event.order.changed.feevalue',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'fee': op.fee.pk,
                ...
                self.order.log_action(
                    'eventyay.event.order.changed.price',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'position': op.position.pk,
                ...
                if isinstance(op.position, OrderPosition):
                    self.order.log_action(
                        'eventyay.event.order.changed.tax_rule',
                        user=self.user,
                        auth=self.auth,
                        data={
                            'position': op.position.pk,
                ...
                self.order.log_action(
                    'eventyay.event.order.changed.cancelfee',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'fee': op.fee.pk,
                ...
                pos = OrderPosition.objects.create(
                    ...
                    order=self.order,
                    tax_rate=op.price.rate,
                    tax_value=op.price.tax,
                    tax_rule=op.product.tax_rule,
                ...
                nextposid += 1
                self.order.log_action(
                    'eventyay.event.order.changed.add',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'position': pos.pk,
                        'product': op.product.pk,
                ...
                tickets.invalidate_cache.apply_async(kwargs={'event': self.event.pk, 'order': self.order.pk})
                self.order.log_action(
                    'eventyay.event.order.changed.secret',
                    user=self.user,
                    auth=self.auth,
                    data={
                        'position': op.position.pk,
    ...
    def _create_split_order(self, split_positions):
        ...
        split_order.log_action(
            ...
            user=self.user,
            auth=self.auth,
            data={'original_order': self.order.code},
        )

        for op in split_positions:
            self.order.log_action(
                'eventyay.event.order.changed.split',
                user=self.user,
                auth=self.auth,
                data={
                    'position': op.pk,
                    'positionid': op.positionid,
                    'old_product': op.product.pk,
        ...
        remaining_total = sum([p.price for p in self.order.positions.all()]) + sum(
            [f.value for f in self.order.fees.all()]
        )
    ...
    def _reissue_invoice(self):
        ...
        if self.reissue_invoice and self._invoice_dirty:
            ...
            if invoice_qualified(self.order) and (
                i
                or self.event.settings.invoice_generate == 'True'
                or (
                    self.open_payment is not None
                    and self.event.settings.invoice_generate == 'paid'
                    and self.open_payment.payment_provider.requires_invoice_immediately
                )
            ):
    ...
    def _clear_tickets_cache(self):
        tickets.invalidate_cache.apply_async(kwargs={'event': self.event.pk, 'order': self.order.pk})
        if self.split_order:
            tickets.invalidate_cache.apply_async(kwargs={'event': self.event.pk, 'order': self.split_order.pk})
...
def _try_auto_refund(
    ...
    if refund_as_giftcard:
        ...
        with transaction.atomic():
            giftcard = order.event.organizer.issued_gift_cards.create(
                ...
                if giftcard_expires is _unset
                else giftcard_expires,
                conditions=giftcard_conditions,
                currency=order.event.currency,
                testmode=order.testmode,
            )
            giftcard.log_action('eventyay.giftcards.created', data={})
    ...
    if notify_admin:
        order.log_action('eventyay.event.order.refund.requested')
    if error:
        raise OrderError(
            _(
...
def cancel_order(
    ...
    try:
        try:
            ...
            if try_auto_refund:
                _try_auto_refund(order, refund_as_giftcard=refund_as_giftcard, comment=comment)
            return ret
        except LockTimeoutException:
            self.retry()
    except (MaxRetriesExceededError, LockTimeoutException):
...
def change_payment_provider(
    ...
    open_fees = list(
        order.fees.annotate(has_p=Exists(e)).filter(Q(fee_type=OrderFee.FEE_TYPE_PAYMENT) & ~Q(has_p=True))
    )
    if open_fees:
        fee = open_fees[0]
        if len(open_fees) > 1:
    ...
    new_fee = payment_provider.calculate_fee(order.pending_sum - old_fee if amount is None else amount)
    if new_fee:
        fee.value = new_fee
    ...
    if create_log and new_payment:
        order.log_action(
            'eventyay.event.order.payment.changed' if open_payment else 'eventyay.event.order.payment.started',
            {
                'fee': new_fee,
                'old_fee': old_fee,
                'provider': payment_provider.identifier,
...
@receiver(order_paid, dispatch_uid='eventyaybase_order_paid_giftcards')
@receiver(order_changed, dispatch_uid='eventyaybase_order_changed_giftcards')
@transaction.atomic()
