...
class PhoneNumberShredder(BaseDataShredder):
    ...
    def shred_data(self):
        ...
        for le in self.event.logentry_set.filter(action_type='eventyay.event.order.phone.changed'):
...
class EmailAddressShredder(BaseDataShredder):
    ...
    def shred_data(self):
        ...
        for le in self.event.logentry_set.filter(action_type='eventyay.event.order.contact.changed'):
            shred_log_fields(le, banlist=['old_email', 'new_email'])

        for le in self.event.logentry_set.filter(action_type='eventyay.event.order.modified').exclude(data=''):
            d = le.parsed_data
            if 'data' in d:
                for row in d['data']:
...
class WaitingListShredder(BaseDataShredder):
    ...
    def generate_files(self) -> List[Tuple[str, str, str]]:
        yield (
            ...
            'application/json',
            json.dumps(
                [WaitingListSerializer(wle).data for wle in self.event.waitinglistentries.all()],
                indent=4,
            ),
        )
    ...
    def shred_data(self):
        ...
        for le in self.event.logentry_set.filter(action_type='eventyay.voucher.added.waitinglist').exclude(data=''):
            d = le.parsed_data
            if 'name' in d:
                d['name'] = '█'
            if 'name_parts' in d:
...
class AttendeeInfoShredder(BaseDataShredder):
    ...
    description = _(
        'This will remove all attendee names and postal addresses from order positions, as well as logged '
        'changes to them.'
    )
    ...
    def generate_files(self) -> List[Tuple[str, str, str]]:
        ...
        for op in OrderPosition.all.filter(order__event=self.event).prefetch_related('answers', 'answers__question'):
            ...
            d[f'{op.order.code}-{op.positionid}'] = AnswerSerializer(
                sorted(op.answers.all(), key=lambda a: a.question_id),
