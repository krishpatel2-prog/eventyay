...
def task_periodic_schedule_export(event_slug):
    ...
    with scopes_disabled():
        event = (
            Event.objects.filter(slug=event_slug).prefetch_related('_settings_objects', 'submissions__slots').first()
        )
    with scope(event=event):
    ...
    for event in events:
        with scope(event=event):
            ...
            if event.current_schedule and event.get_feature_flag('export_html_on_release'):
                task_periodic_schedule_export.apply_async(args=(event.slug,), ignore_result=True)
