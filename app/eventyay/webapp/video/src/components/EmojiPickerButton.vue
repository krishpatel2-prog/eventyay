...
.c-emoji-picker-button
	bunt-icon-button.btn-emoji-picker(ref="button", :tooltip="$t('Insert emoji')", tooltip-placement="top-start", @click="toggle")
...
export default {
	...
	methods: {
		...
		selected(emoji) {
			// Don't close immediately, emit the selected emoji and reset timer
		...
		resetDismissTimer() {
			...
			this.clearDismissTimer()
			// Set new timer to auto-dismiss after 1.5 seconds of no interaction
