...
export function autoWrapPasswordFields(root = document) {
    ...
    root.querySelectorAll('input[type="password"]').forEach(input => {
        ...
        btn.innerHTML = `
            ...
            <svg class="icon-eye-slash icon-eye-slash--hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        btn.addEventListener('click', function(e) {
            ...
            if (input && iconEye && iconEyeSlash) {
                ...
                if (input.getAttribute('type') === 'password' || input.type === 'password') {
                    ...
                    input.type = 'text';
                    iconEye.classList.add('icon-eye--hidden');
                    iconEyeSlash.classList.remove('icon-eye-slash--hidden');
                    btn.setAttribute('aria-pressed', 'true');
                    btn.setAttribute('aria-label', hideLabel);
                } else {
                    input.setAttribute('type', 'password');
                    input.type = 'password';
                    iconEye.classList.remove('icon-eye--hidden');
                    iconEyeSlash.classList.add('icon-eye-slash--hidden');
                    btn.setAttribute('aria-pressed', 'false');
                    btn.setAttribute('aria-label', showLabel);
                }
            }
